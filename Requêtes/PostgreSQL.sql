...
FROM
    P17_Nageurs
WHERE
    NageurNom ~* '^(?=.*[A-Z]{2})(?!.*[aeiou]{4})(?=.{10,})(?!.*(.)\\1{2})[A-Za-z0-9]{5,10}$';
...
Analyse des différences
    ...
    La syntaxe explicite :  avec INNER JOIN est la plus rapide (8 ms) car elle permet au moteur SQL d'optimiser directement les jointures.
    Le traitement se limite uniquement aux lignes ayant des correspondances entre les tables.

    La syntaxe implicite (13 ms) :  est légèrement moins performante car le moteur SQL doit interpréter les conditions dans la clause WHERE pour éviter un produit cartésien complet.

    La jointure externe (LEFT JOIN, 12 ms) :  est plus lente que l'INNER JOIN car elle inclut des lignes non correspondantes et génère des valeurs NULL.
    Ce traitement supplémentaire alourdit légèrement le temps d'exécution.

    Le produit cartésien avec restriction (13 ms) :  est inefficace en apparence, mais le moteur SQL moderne optimise cette requête pour obtenir des performances similaires à celles de la syntaxe implicite.
*/
...
-- POURQUOI LA JOINTURE INTERNE EST EXTERNE RENVOIENT LE MÊME RÉSULTAT ??
...
1-		Version 2 (INNER JOIN) est la plus rapide (13 ms) :
    Les jointures explicites permettent au moteur SQL d'optimiser directement les relations entre les tables, sans passer par un produit cartésien implicite.

2-		Version 3 (RIGHT JOIN) est la plus lente (16 ms) :
    Les RIGHT JOIN incluent initialement tous les nageurs, même ceux sans participations, avant d'appliquer les conditions.

3-		Version 4 (Produit cartésien) est plus lente que la syntaxe explicite mais légèrement plus rapide que la syntaxe implicite (14 ms) :
    Le produit cartésien initial est coûteux en théorie, mais les moteurs SQL modernes optimisent souvent ce type de requête, ce qui limite l'impact.
...
-- Utilisation de `NOT IN` pour exclure les nageurs français
...
-- PostgreSQL : Utilisation de `LEFT JOIN` pour exclure les nageurs français
SELECT
    n1.NageurNom,
    n1.NageurPrenom,
    n1.NageurPays
...
FROM (
         SELECT
             ...
             AVG(
                     CASE
                         WHEN Temps ~ '^\d+:\d{2}\.\d{2}$' THEN
                             CAST(SPLIT_PART(Temps, ':', 1) AS FLOAT) * 60 + -- Minutes
                             CAST(SPLIT_PART(SPLIT_PART(Temps, ':', 2), '.', 1) AS FLOAT) + -- Secondes
                             CAST(SPLIT_PART(Temps, '.', 2) AS FLOAT) / 100 -- Millisecondes
                         ELSE 0 -- Temps invalide remplacé par 0
                         END
             ) AS TempsMoyen
         FROM
             P17_ParticiperIndividuel
         WHERE
             Temps IS NOT NULL -- Exclure les valeurs NULL
         GROUP BY
             EpreuveID
     ) T;
...
La version avec jointure est clairement la plus efficace, car :
   ...
   - La requête avec jointure est plus rapide (5 ms) que la requête avec sous-requête (10 ms).
   - Cela s’explique par le fait que la jointure permet de traiter toutes les données en une seule passe, alors que la sous-requête corrélée multiplie le nombre de calculs.
...
SELECT
    ...
    N.NageurPrenom,
    AVG(
            CAST(SPLIT_PART(P.Temps, ':', 1) AS FLOAT) * 60 + -- Minutes
            CAST(SPLIT_PART(SPLIT_PART(P.Temps, ':', 2), '.', 1) AS FLOAT) + -- Secondes
            CAST(SPLIT_PART(P.Temps, '.', 2) AS FLOAT) / 100 -- Millisecondes
    ) AS TempsMoyen
FROM
    P17_ParticiperIndividuel P
        NATURAL JOIN P17_Nageurs N
WHERE
    P.Temps != '0:00.00' -- Exclure les temps non significatifs
GROUP BY
...
FROM
    P17_ParticiperIndividuel
WHERE
    Temps != '0:00.00' -- Exclure les temps non significatifs
GROUP BY
    EpreuveID;
...
/*
    ...
    CAST(SPLIT_PART(Temps, ':', 1) AS FLOAT) * 60 : Convertit les minutes en secondes.
    CAST(SPLIT_PART(SPLIT_PART(Temps, ':', 2), '.', 1) AS FLOAT) : Ajoute les secondes entières.
    CAST(SPLIT_PART(Temps, '.', 2) AS FLOAT) / 100 : Ajoute les fractions de seconde.
...
HAVING
    AVG(
            CAST(SPLIT_PART(Temps, ':', 1) AS FLOAT) * 60 +
            CAST(SPLIT_PART(SPLIT_PART(Temps, ':', 2), '.', 1) AS FLOAT) +
            CAST(SPLIT_PART(Temps, '.', 2) AS FLOAT) / 100
    ) < 60; -- Garder uniquement les nageurs avec un temps moyen < 60s
